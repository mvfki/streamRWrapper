...
adata2sce <- function(AnnData) {
    ...
    # Metadata
    unsNames <- names(AnnData$uns)
    pythonClass <- "python.builtin.object"
    for (i in 1:length(unsNames)) {
        key <- unsNames[i]
        value = AnnData$uns[[key]]
        if (sort(class(value) == pythonClass, decreasing = TRUE)[1]) {
            tryResult <- try(value$to_list(), silent = TRUE)
            if (!class(tryResult) == "try-error") {
                value <- tryResult
            }
        }
        sce@metadata[[key]] <- value
    }
    return(sce)
}
